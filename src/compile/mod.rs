...
use crate::{
    ...
    tokenize::{TokenizationError, TokenizationErrorKind, Tokenizer},
};
                    ...
                    errors.iter().for_each(|e| {
                        ...
                        let final_report = match &e.kind {
                            ...
                            ParsingErrorKind::ExpectedAnExpressionButFound(token) => report
                                ...
                                .with_label(label.with_message(format!("Expected an expression but instead found token \"{}\"", token_kind_to_string(&token.kind, &self.interners)))),
                            ParsingErrorKind::ExpectedATypeButFound(token) => report
                                .with_message("Expected a type")
                                .with_label(label.with_message(format!("Expected a type but instead found token \"{}\"", token_kind_to_string(&token.kind, &self.interners)))),
                            ParsingErrorKind::InvalidSuffixOperator(token) => report
                                .with_message("Invalid suffix operator")
                                .with_label(label.with_message(format!("Invalid token as expression suffix operator \"{}\"", token_kind_to_string(&token.kind, &self.interners)))),
                            ...
                            ParsingErrorKind::UnknownStaticMethod(identifier_node) => {
                                let name = self.interners.string_interner.resolve(identifier_node.name);
                            ...
                            ParsingErrorKind::ExpectedStatementOrExpression { found } => report
                                ...
                                .with_label(label.with_message(format!("Expected a statement or an expression but instead found token \"{}\"", token_kind_to_string(&found.kind, &self.interners)))),
                            ParsingErrorKind::UnexpectedTokenAfterFinalExpression { found } => report
                                .with_message("Unexpected token after final expression")
                                .with_label(label.with_message(format!("Unexpected token \"{}\" after final expression", token_kind_to_string(&found.kind, &self.interners)))),
                        ...
                        let final_report = match &e.kind {
                            ...
                            SemanticErrorKind::CannotCompareType { of, to } => {
                                report.with_message("Cannot compare types")
                                    .with_label(label.with_message(format!("Cannot compare type \"{}\" to type \"{}\"", type_to_string(&of, &self.interners), type_to_string(&to, &self.interners))))
                            }
                            SemanticErrorKind::UndeclaredIdentifier(id) => {
                                let name = self.interners.string_interner.resolve(id.name);
                            ...
                            SemanticErrorKind::TypeMismatch { expected, received } => {
                                let expected_type_str = type_to_string(&expected, &self.interners);
                                let received_type_str = type_to_string(&expected, &self.interners);

                                report.with_message("Type mismatch").with_label(label.with_message(format!("Type mismatch, expected `{}`, instead found `{}`", expected_type_str, received_type_str)))
                            }
                            ...
                            SemanticErrorKind::ReturnTypeMismatch { expected, received } => {
                                report.with_message("Return type mismatch")
                                    .with_label(label.with_message(format!("Expected the return value to be assignable to {}, found {}", type_to_string(&expected, &self.interners), type_to_string(&received, &self.interners))))
                            }
                            SemanticErrorKind::CannotAccess(target) => report
                                .with_message("Cannot access field")
                                .with_label(label.with_message(format!("Cannot use the access operator on the type \"{}\"", type_to_string(&target, &self.interners)))),
                            SemanticErrorKind::CannotCall(target) => report
                                .with_message("Cannot use the function call operator")
                                .with_label(label.with_message(format!("Cannot use the function-call operator on type \"{}\"", type_to_string(&target, &self.interners)))),
                            ...
                            SemanticErrorKind::AccessToUndefinedField(field) => {
                                let name = self.interners.string_interner.resolve(field.name);
                            ...
                            SemanticErrorKind::MissingStructFieldInitializers(missing_fields) => {
                                let field_names: Vec<String> = missing_fields.into_iter().map(|f| self.interners.string_interner.resolve(*f)).collect();
                            ...
                            SemanticErrorKind::DuplicateIdentifier(id) => {
                                let identifier_name = self.interners.string_interner.resolve(id.name);
                            ...
                            SemanticErrorKind::ClosuresNotSupportedYet => todo!(),
                        };
