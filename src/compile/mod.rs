        ...
        rayon::scope(|s| {
            while let Some(source_path) = work_queue.lock().unwrap().pop_front() {
                ...
                s.spawn(|_| {
                    let source_code = match fs::read_to_string(&source_path) {
                        ...
                        Err(e) => {
                            all_results
                                .lock()
                                .unwrap()
                                .push(Err(CompilationError::CouldNotReadFile {
                                    path: source_path,
                                    error: e,
                                }));
                            return;
                    ...
                    let mut local_results: Vec<Result<ParallelParseResult, CompilationError>> =
            ...
            match error {
                                ...
                                let diagnostic = match &e.kind {
                                    ...
                                    ParsingErrorKind::ExpectedAnExpressionButFound(token) => {
                                        err.with_message("Expected an expression")
                                            .with_label(label.with_message(format!(
                                                ...
                                                token_kind_to_string(&token.kind, self.interner.clone())
                                            )))
                                    }
                                    ...
                                    ParsingErrorKind::ExpectedStatementOrExpression { found } => err
                                        ...
                                        .with_label(label.with_message(format!(
                                            ...
                                            token_kind_to_string(&found.kind, self.interner.clone())
                                        ))),
                                ...
                                let diagnostic = match &e.kind {
                                    ...
                                    SemanticErrorKind::CannotCompareType { of, to } => {
                                                                        err.with_message("Cannot compare types")
                                                                            .with_label(label.with_message(format!(
                                                                                ...
                                                                                type_to_string(&of.kind, self.interner.clone()),
                                                                                type_to_string(&to.kind, self.interner.clone())
                                                                            )))
                                                                    }
                                    ...
                                    SemanticErrorKind::TypeMismatch { expected, received } => {
                                                                        ...
                                                                        err.with_message("Type mismatch").with_labels(vec![
                                                                            label.with_message(format!(
                                                                                ...
                                                                                constraint_str,
                                                                                type_to_string(&received.kind, self.interner.clone())
                                                                            )),
                                                                            Label::secondary(file_id, declaration_of_expected)
                                    ...
                                    SemanticErrorKind::ReturnTypeMismatch { expected, received } => {
                                                                        err.with_message("Return type mismatch")
                                                                            .with_label(label.with_message(format!(
                                                                                ...
                                                                                type_to_string(&expected.kind, self.interner.clone()),
                                                                                type_to_string(&received.kind, self.interner.clone())
                                                                            )))
                                                                    }
                                    SemanticErrorKind::CannotAccess(target) => {
                                                                        err.with_message("Cannot access field").with_label(label.with_message(format!(
                                                                            ...
                                                                            type_to_string(&target.kind, self.interner.clone())
                                                                        )))
                                                                    }
                ...
                CompilationError::CouldNotReadFile { path, error } => {
                    let diagnostic = Diagnostic::error()
                        ...
                        .with_notes(vec![format!("Path: {}", path.display()), format!("Error: {}", error)]);
                    let _ = term::emit(&mut writer.lock(), &config, &*files, &diagnostic);
                }
                CompilationError::ModuleNotFound {
                    ...
                    let diagnostic = Diagnostic::error().with_message("Module not found").with_notes(vec![
                        format!("Could not resolve import '{}'", target_path.display()),
