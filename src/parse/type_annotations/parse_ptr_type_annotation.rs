use crate::{
    ast::type_annotation::{TypeAnnotation, TypeAnnotationKind},
    parse::{Parser, ParsingError},
    tokenize::{KeywordKind, PunctuationKind},
};

impl Parser {
    pub fn parse_ptr_type_annotation(
        &mut self,
        is_mutable: bool,
    ) -> Result<TypeAnnotation, ParsingError> {
        let start_offset = self.offset;

        if is_mutable {
            self.consume_keyword(KeywordKind::Mut)?;
        } else {
            self.consume_keyword(KeywordKind::Ref)?;
        }

        self.consume_punctuation(PunctuationKind::Lt)?;
        let inner_type = self.parse_type_annotation(0)?;
        self.consume_punctuation(PunctuationKind::Gt)?;

        let span = self.get_span(start_offset, self.offset - 1)?;

        let kind = if is_mutable {
            TypeAnnotationKind::Mut(Box::new(inner_type))
        } else {
            TypeAnnotationKind::Ref(Box::new(inner_type))
        };

        Ok(TypeAnnotation { kind, span })
    }
}
