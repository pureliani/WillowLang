...
impl<'a, 'b> Parser<'a, 'b> {
    pub fn parse_tag_type_annotation(&mut self) -> Result<TypeAnnotation, ParsingError<'a>> {
        ...
        loop {
            ...
            let value_type = if self.match_token(0, crate::tokenize::TokenKind::Punctuation(PunctuationKind::LParen)) {
        ...
        if tags.len() > 1 {
            ...
            Ok(TypeAnnotation {
                kind: TypeAnnotationKind::Tag(tags.pop().expect("Expected at least single tag item")),
                span,
            })
        }
    }
